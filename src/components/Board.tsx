...
const findNewBoardState = ({ apple, snakeHead, snakeTail, mines, isWalls, boardSize }: Props) => {
    const newBoardState = new Array<CellColor>(boardSize.x * boardSize.y).fill(ELEMENTS_COLORS.empty);
    newBoardState[coordsToInt(apple, boardSize.x)] = ELEMENTS_COLORS.apple;
    snakeTail.forEach(tail => newBoardState[coordsToInt(tail, boardSize.x)] = ELEMENTS_COLORS.tail);
    mines.map((coords) => coordsToInt(coords, boardSize.x)).forEach(mine => newBoardState[mine] = ELEMENTS_COLORS.mine);
    if ((isWalls && isWithinGrid(snakeHead, boardSize)) || !isWalls) {
        newBoardState[coordsToInt(snakeHead, boardSize.x)] = ELEMENTS_COLORS.head;
    }
